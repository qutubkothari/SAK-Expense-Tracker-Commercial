...
class CurrencyConverter {
  constructor(supabase, userBaseCurrency = 'USD') {
    this.supabase = supabase;
    this.rates = {};
    this.baseCurrency = userBaseCurrency; // Dynamic base currency per user
  }
  
  // Update base currency (call when user changes preference)
  setBaseCurrency(currency) {
    this.baseCurrency = currency;
    console.log(`Base currency updated to: ${currency}`);
  }
  ...
  convertToBase(amount, fromCurrency) {
    ...
    const key = `${fromCurrency}_${this.baseCurrency}`;
    const rate = this.rates[key];
    
    if (!rate) {
      console.warn(`No exchange rate found for ${fromCurrency} to ${this.baseCurrency}, using 1:1`);
      return amount;
    }
  ...
  getRate(fromCurrency, toCurrency = null) {
    // Use base currency if no target specified
    const targetCurrency = toCurrency || this.baseCurrency;
    
    if (fromCurrency === targetCurrency) return 1.0;
    
    const key = `${fromCurrency}_${targetCurrency}`;
  ...
  async fetchLiveRates() {
    try {
      // Using exchangerate-api.com free tier - fetch rates based on user's base currency
      const response = await fetch(`https://api.exchangerate-api.com/v4/latest/${this.baseCurrency}`);
      const data = await response.json();
      
      if (data && data.rates) {
        // Convert rates to "X to base" format
        const currencies = ['AED', 'USD', 'EUR', 'GBP', 'SGD', 'INR', 'SAR', 'QAR', 'KWD', 'OMR', 'BHD', 'PKR', 'BDT', 'LKR'];
        
        for (const currency of currencies) {
          if (currency === this.baseCurrency) continue; // Skip base currency
          
          if (data.rates[currency]) {
            // Since API gives base to X, we need X to base
            const rate = 1 / data.rates[currency];
            await this.updateRate(currency, this.baseCurrency, rate);
          }
        }
        
        console.log(`Live rates updated successfully for base currency: ${this.baseCurrency}`);
        return true;
      }
    } catch (error) {
  ...
  // Detect currency from location keywords (generic, not region-specific)
  detectCurrency(text) {
    const lowerText = text.toLowerCase();
    
    // Generic currency keyword detection
    ...
    // Default to user's base currency
    return { currency: this.baseCurrency, keyword: null };
  }
