...
class VoiceAI {
  ...
  async startRecording() {
    try {
      ...
      // Request microphone permission if in Capacitor
      if (isCapacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.Device) {
        try {
          // Request permission using Capacitor's permission API
          const permission = await navigator.permissions.query({ name: 'microphone' });
          console.log('🎤 Microphone permission status:', permission.state);
          
          if (permission.state === 'denied') {
            throw new Error('Microphone permission denied. Please enable it in app settings.');
          }
        } catch (permError) {
          console.warn('⚠️ Permission check failed:', permError);
          // Continue anyway - getUserMedia will prompt for permission
        }
      }
  ...
  async transcribeAudio(audioBlob, userLanguage = null) {
    try {
      ...
      const proxyUrl = 'https://us-central1-sak-expense-tracker.cloudfunctions.net/openaiProxy';
...
🚨 CRITICAL: Extract amount EVEN IF date keywords (yesterday/today/last week) are present!
Examples:
- "1200 rupees pizza hut yesterday" → amount: 1200 (NOT 0!)
- "500 taxi yesterday" → amount: 500 (NOT 0!)
- "40 lunch last week" → amount: 40 (NOT 0!)
...
- "1200 rupees pizza hut yesterday" → note: "pizza hut" (remove "1200 rupees" and "yesterday")
- "lunch 40 2 days ago" → note: "lunch" (remove "2 days ago" and "40")

STEP 5 - DATE EXTRACTION (INTELLIGENT RELATIVE DATES):
Today is ${new Date().toISOString().split('T')[0]}
🚨 Process date INDEPENDENTLY - don't let it affect amount/currency extraction!
...
- "1200 rupees pizza hut yesterday" → date: "${(() => { const d = new Date(); d.setDate(d.getDate() - 1); return d.toISOString().split('T')[0]; })()}" (yesterday, amount=1200, currency=INR)
...
4. "500rs darees yesterday" → {"transaction_type": "expense", "amount": 500, "currency": "INR", "note": "darees", "location": "India", "date": "${(() => { const d = new Date(); d.setDate(d.getDate() - 1); return d.toISOString().split('T')[0]; })()}", ...}
5. "1200 rupees pizza hut yesterday" → {"transaction_type": "expense", "amount": 1200, "currency": "INR", "note": "pizza hut", "location": "India", "date": "${(() => { const d = new Date(); d.setDate(d.getDate() - 1); return d.toISOString().split('T')[0]; })()}", ...}
6. "lunch 40 2 days ago" → {"transaction_type": "expense", "amount": 40, "currency": "${userDefaultCurrency}", "note": "lunch", "location": null, "date": <2 days ago>, ...}
7. "coffee 5 london" → {"transaction_type": "expense", "amount": 5, "currency": "GBP", "note": "coffee", "location": "London", "date": "${new Date().toISOString().split('T')[0]}", ...}

🚨 CRITICAL REMINDERS:
- NEVER return amount=0 if amount is in transcript!
- Date keywords (yesterday/last week) should NOT affect amount/currency extraction
- Extract amount, currency, and note FIRST, then handle date separately

Return ONLY valid JSON, no markdown.`;

      console.log('📡 Sending to GPT-4...');
      
      const response = await fetch('https://us-central1-sak-expense-tracker.cloudfunctions.net/openaiProxy', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
